...
from utils_isbi import add_scalars_rec_volume,add_scalars_reg,save_best_network,save_best_network_reg
...
def ConvPose(labels,ori_pts,pre, option_method,device):
    ...
    return labels_opt,pred_pts_opt
...
class Train_Rec_Reg_Model():
            ...
            for step, (frames, tforms, tforms_inv) in enumerate(self.train_loader_rec):
                ...
                if self.opt.Loss_type == "MSE_points":
                    ...
                    if self.opt.ddf_dirc == 'Move' and self.opt.Conv_Coords == 'optimised_coord':
                        ...
                        # with torch.no_grad():
                        wrap_loss,wrap_dist,pred_pts_warped = wrapped_pred_dist(ddf,pred_pts,labels,self.option,frames.shape[2],frames.shape[3],None,None,self.device)
                ...
                for i in range(gt_volume.shape[0]):
                    save2mha(gt_volume[i,...].cpu().numpy(),sx = 1,sy=1,sz=1,
                        save_folder='gt_train_%d.mha'%i
                        )
                    save2mha(pred_volume[i,...].detach().cpu().numpy(),sx = 1,sy=1,sz=1,
                        save_folder='pred-train_%d.mha'%i
                        )
                    save2mha(warped[i,0,...].detach().cpu().numpy(),sx = 1,sy=1,sz=1,
                        save_folder='warp-train_%d.mha'%i
                        )
            ...
            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.val_fre):
                ...
                for step, (fr_val, tf_val, tf_val_inv) in enumerate(self.val_loader_rec):
                    ...
                    if self.opt.Conv_Coords == 'optimised_coord':
                        ...
                        labels_val, pred_pts_val = ConvPose(labels_val, ori_pts_val, pre_val, 'auto_PCA',self.device)
        ...
        for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.NUM_EPOCHS):
            ...
            for step, (frames, tforms, tforms_inv) in enumerate(self.train_loader_reg):
                ...
                if self.opt.ddf_dirc == 'Fix':
                    ...
                    loss = self.img_loss(torch.squeeze(warped,1),pred_volume) + self.regularization(ddf)
    ...
    def load_best_rec_model_epoch500(self):
        # load the best transformation model for training again or for generating volume data for registartion model use
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_epoch00003000'),map_location=torch.device(self.device)))
        except:
            print('No best rec model saved at the moment...')
