...
class SSFrameDataset():  # Subject-Scan frame loader
    ...
    def __init__(self, min_scan_len, filename_h5, indices_in_use=None, num_samples=2, sample_range=None):
        ...
        # [()] means returns as a numpy array, otherwise returns as a h5py dataset object
        ...
        self.num_sub = self.num_frames.shape[0]
    ...
    def partition_by_ratio(self, ratios, randomise=False, subject_level=False):
        ...
        if subject_level: # subject-level split
            # raise('Subject-level split has not been implemented.')  # N.B. TBA
            # added by Qi
            set_sizes = [int(self.num_sub*r) for r in ratios]
            sub_ind = list(range(self.num_sub))
            for ii in range(self.num_sub-sum(set_sizes)): 
                set_sizes[ii]+=1
            if randomise:
                # obtain the same split result, to give the same training and validation set
            ...
            # indices_sets = [self.indices_in_use[
            
            # idx_all = [np.where(np.array(fir_ele)==indices_sets_sub[i_sub][i_sub_sub]) for i_sub in range(len(indices_sets_sub)) for i_sub_sub in range(len(indices_sets_sub[i_sub]))]
            ...
            return [SSFrameDataset(min_scan_len = self.min_scan_len, filename_h5=self.filename, indices_in_use=idx_list, num_samples=self.num_samples, sample_range=self.sample_range) for idx_list in indices_sets]
            ...
            if randomise:
                # obtain the same split result, to give the same training and validation set
                random.Random(4).shuffle(self.indices_in_use)
    ...
    def __add__(self, other):
        ...
        return SSFrameDataset(min_scan_len = self.min_scan_len,filename_h5=self.filename, indices_in_use=indices_combined, num_samples=self.num_samples, sample_range=self.sample_range)
    ...
    def __getitem__(self, idx):
        ...
        return np.stack(frames,axis=0), np.stack(tforms,axis=0), np.stack(tforms_inv,axis=0),indices
    ...
    def read_json(jason_filename, num_samples=None):
        with open(jason_filename, 'r', encoding='utf-8') as f:
            ...
            return SSFrameDataset(
                ...
                filename_h5    = '/home/qili/workspace/frames_res4.h5',#obj['filename'],
