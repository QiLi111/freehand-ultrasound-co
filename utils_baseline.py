...
def save_best_network(opt, model, epoch_label, running_loss_val, running_dist_val, val_loss_min, val_dist_min,
    linear_list_anatomy,linear_list_protocol,params_cla_anatomy,feature_map):
    '''
    ...
    if running_loss_val < val_loss_min:
        ...
        torch.save(params_cla_anatomy, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_loss_weights'))
    ...
    if running_dist_val < val_dist_min:
        ...
        torch.save(params_cla_anatomy, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_dist_weights'))
...
def add_scalars_cls_each_model(writer,epoch, loss_anatomy,acc_anatomy,loss_anatomy_train,acc_anatomy_train):
    keys = list(loss_anatomy.keys())
    ...
    for i in range(len(keys)):
        ...
        # writer.add_scalars('acc_each_model', {'training_protocol_model_%s' % str(i)  : acc_protocol_train[f'module{i}']}, epoch)
        writer.add_scalars('loss_each_model', {'training_anatomy_model_%s' % str(i)  : loss_anatomy_train[keys[i]]}, epoch)
        writer.add_scalars('f1_each_model', {'training_anatomy_model_%s' % str(i)  : acc_anatomy_train[keys[i]]}, epoch)
        ...
        # writer.add_scalars('acc_each_model', {'val_protocol_model_%s' % str(i)  : acc_protocol[f'module{i}']}, epoch)
        writer.add_scalars('loss_each_model', {'val_anatomy_model_%s' % str(i)  : loss_anatomy[keys[i]]}, epoch)
        writer.add_scalars('f1_each_model', {'val_anatomy_model_%s' % str(i)  : acc_anatomy[keys[i]]}, epoch)





def add_scalars_cls_weighted(writer,epoch,loss_anatomy,acc_anatomy,loss_anatomy_train,acc_anatomy_train):
    # classification tasks on validation set, for tarining process
    ...
    # writer.add_scalars('acc_weighted', {'training_protocol_acc':acc_protocol_train}, epoch)
    ...
    # writer.add_scalars('acc_weighted', {'val_protocol_acc':acc_protocol}, epoch)
...
def add_scalars_weights(writer,epoch,params_cla_anatomy):
    for i in range(len(params_cla_anatomy)):
...
def add_scalars_weights_acc(writer,params_cla_anatomy,acc_anatomy,acc_anatomy_train):
    for i in range(len(params_cla_anatomy)):
        # classification tasks on validation set, for tarining process
        # writer.add_scalars('acc_w', {'training_protocol_model_%s' % str(i)  : acc_protocol_train[f'module{i}']}, params_cla_protocol[i])
        writer.add_scalars('f1_w', {'training_anatomy_model_%s' % str(i)  : acc_anatomy_train[f'module{i}']}, params_cla_anatomy[i])
        
        # writer.add_scalars('acc_w', {'val_protocol_model_%s' % str(i)  : acc_protocol[f'module{i}']}, params_cla_protocol[i])
        writer.add_scalars('f1_w', {'val_anatomy_model_%s' % str(i)  : acc_anatomy[f'module{i}']}, params_cla_anatomy[i])


def add_scalars_weights_loss(writer,params_cla_anatomy,loss_anatomy,loss_anatomy_train):
    for i in range(len(params_cla_anatomy)):
        # classification tasks on validation set, for tarining process
