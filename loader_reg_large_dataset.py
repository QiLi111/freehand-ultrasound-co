...
class SSFrameDataset_reg():  # Subject-Scan frame loader

    def __init__(self, min_scan_len, h5_file_name, indices_in_use=None, num_samples=2, sample_range=None):
        ...
        self.filename = h5_file_name
        self.file = h5py.File(self.filename, 'r')
        self.frame_size = None #self.file['frame_size'][()]
    ...
    def partition_by_ratio(self, ratios, randomise=False, subject_level=False):
        ...
        if subject_level:
            ...
            return [SSFrameDataset_reg(min_scan_len = self.min_scan_len, h5_file_name=self.h5_file_name, indices_in_use=idx_list, num_samples=self.num_samples, sample_range=self.sample_range) for idx_list in indices_sets]
    ...
    def __add__(self, other):
        ...
        return SSFrameDataset_reg(min_scan_len = self.min_scan_len, h5_file_name=self.h5_file_name, indices_in_use=indices_combined, num_samples=self.num_samples, sample_range=self.sample_range)
    ...
    def __getitem__(self, idx):
        ...
        i_seq = random.randint(0,self.num_frames[indices]-self.num_samples+1)

        gt_volume = self.file['/sub{:03d}_scan{:02d}_seq{:04d}_gt'.format(indices[0],indices[1],i_seq)] 
        pred_volume = self.file['/sub{:03d}_scan{:02d}_seq{:04d}_pred'.format(indices[0],indices[1],i_seq)] 
    ...
    @staticmethod
    def read_json(jason_filename, h5_file_name, num_samples=None):
        with open(jason_filename, 'r', encoding='utf-8') as f:
            obj = json.load(f)
            return SSFrameDataset_reg(
