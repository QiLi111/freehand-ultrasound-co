...
class Visualizer_plot_volume():  # plot scan
    ...
    def generate_volume_data(self, scan_index,saved_folder,based_volume = 'common_volume',local_global = 'global', pts_num = 'all'):
        ...
        #     points_each_frm = self.all_points
        # elif local_global == 'local':
        #     points_each_frm = self.four_pts
        # else:
        #     raise('Not implement')
        ...
        # ori_pts = torch.matmul(tforms_each_frame2frame0_gt_all,torch.matmul(self.tform_calib,self.all_points)).permute(0,1,3,2)
        ...
        self.plot_scan(labels_gt_four,pred_pts_four,frames[:,100:labels_gt_four.shape[1]+100,...],saved_folder+'/'+saved_name+'_T_global')
        self.plot_scan(labels_gt_four,pred_pts_warped_four,frames[:,100:labels_gt_four.shape[1]+100,...],saved_folder+'/'+saved_name+'_TR_global')
        self.plot_scan(labels_gt_four_local,pred_pts_four_local,frames[:,100:labels_gt_four.shape[1]+100,...],saved_folder+'/'+saved_name+'_T_local')
        ...
        if generate_mha:
            ...
            save2mha(pred_volume_warp[0,...].detach().cpu().numpy(),sx = 1,sy=1,sz=1,
                save_folder=saved_folder+'/'+'wrapped-test.mha'
                )
...
# plt.savefig('./test.png')
    ...
    def plot_scan(self,labels_gt_four,pred_pts_four,frames,saved_name):
        ...
        with open(all_gt_fd+'/'+saved_name.split('/')[-1]+'_gt_common.npy', 'wb') as f:
            np.save(f, labels_gt_four.cpu().numpy())
        with open(saved_name+'_pred_common.npy', 'wb') as f:
            np.save(f, pred_pts_four.cpu().numpy())
        with open(all_frames_fd+'/'+saved_name.split('/')[-1]+'_frame_common.npy', 'wb') as f:
