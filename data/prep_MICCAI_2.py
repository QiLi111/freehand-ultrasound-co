...
fh5_frames = h5py.File(os.path.join(PATH_SAVE,'frames_res{}_cls_6'.format(RESAMPLE_FACTOR)+".h5"),'a')
if FLAG_SCANFILE:
    fh5_scans = h5py.File(os.path.join(PATH_SAVE,'scans_res{}_cls_6'.format(RESAMPLE_FACTOR)+".h5"),'a')
if Extract_Feats:
    fh5_feats = h5py.File(os.path.join(PATH_SAVE,'frame_feats_res{}_cls_6'.format(RESAMPLE_FACTOR)+".h5"),'a')
...
cls_protocol = np.array([0,1,2,3,4,5])
...
for i_sub, folder in enumerate(folders_subject):
    ...
    for i_scan, fn in enumerate(fn_mha):
        ...
        if 'Linear' in name_scan[i_sub][i_scan] and 'Para' in name_scan[i_sub][i_scan]:
            protocol_cls = cls_protocol[0]
        elif 'Linear' in name_scan[i_sub][i_scan] and 'Ver' in name_scan[i_sub][i_scan]:
            protocol_cls = cls_protocol[1]

        elif 'C' in name_scan[i_sub][i_scan] and 'Para' in name_scan[i_sub][i_scan]:
            protocol_cls = cls_protocol[2]
        elif 'C' in name_scan[i_sub][i_scan] and 'Ver' in name_scan[i_sub][i_scan]:
            protocol_cls = cls_protocol[3]


        elif 'S' in name_scan[i_sub][i_scan] and 'Para' in name_scan[i_sub][i_scan]:
            protocol_cls = cls_protocol[4]

        elif 'S' in name_scan[i_sub][i_scan] and 'Ver' in name_scan[i_sub][i_scan]:
            protocol_cls = cls_protocol[5]
