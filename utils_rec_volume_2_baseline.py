...
from utils_isbi import add_scalars_rec_volume,add_scalars_reg,save_best_network,save_best_network_reg,add_scalars_wrap_dist
...
def calculateConvPose_batched1(pts_batched,option,device):
    for i_batch in range(pts_batched.shape[0]):
...
class Train_Rec_Reg_Model():
    ...
    def __init__(
        ...
        # self.train_dist_min_T = val_loss_min
        # self.train_dist_min_R = val_loss_min
        ...
        # self.train_loader_reg = self.train_loader_rec
        ...
        # self.val_loader_reg = self.val_loader_rec
    ...
    def train_rec_model(self):
        ...
        # for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.max_rec_epoch_each_interation):
        for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.NUM_EPOCHS):
            ...
            for step, (frames, tforms, tforms_inv) in enumerate(self.train_loader_rec):
                ...
                if self.opt.Loss_type == "MSE_points":
                    ...
                    if self.opt.ddf_dirc == 'Move' and self.opt.Conv_Coords == 'optimised_coord':
                        ...
                        # for test
                        # pred_pts_warped_sub = pred_pts_warped.permute(0,1,3,2)
                        # pred_pts_warped_sub_pad = F.pad(input=pred_pts_warped_sub, pad=(0, 1, 0, 0), mode='constant', value=1)
                        # pred_pts_warped=pred_pts_warped_sub_pad[:,:,:,:3].permute(0,1,3,2)
                        
                        # gt_volume,wraped_pred_volume, warped, ddf = self.scatter_pts_registration(labels,pred_pts_warped,frames,step)
                ...
                # pred_pts.retain_grad()
                # pred_transfs.retain_grad()
                if epoch !=0:
                    loss.backward()
                    self.optimiser_rec_reg.step()

                # plot gradient
                # plt.imshow(pred_pts.grad[0,10,...].detach().cpu().numpy())
            ...
            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.val_fre):
                ...
                for step, (fr_val, tf_val, tf_val_inv) in enumerate(self.val_loader_rec):
                    ...
                    # dist_val = self.metrics(pred_pts_val, labels_val)
                ...
                # self.save_best_models_train_T(epoch,dist)
                # self.save_best_models_train_R(epoch,train_epoch_wrap_dist)
                ...
                # self.val_loss_min, self.val_dist_min = save_best_network(self.opt, self.model, epoch, epoch_loss_val_rec, epoch_dist_val, self.val_loss_min, self.val_dist_min)
                
                # self.val_loss_min_reg,count_non_improved_loss_reg = save_best_network_reg(self.opt, self.VoxelMorph_net, epoch, epoch_loss_val_reg, self.val_loss_min_reg,count_non_improved_loss_reg)
    ...
    def train_reg_model(self):
        ...
        for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.NUM_EPOCHS):
            ...
            train_epoch_dist = 0
            train_epoch_loss_reg = 0
            for step, (frames, tforms, tforms_inv) in enumerate(self.train_loader_reg):
                frames, tforms, tforms_inv = frames.to(self.device), tforms.to(self.device), tforms_inv.to(self.device)
                ...
                if self.opt.Conv_Coords == 'optimised_coord' and self.opt.img_pro_coord == 'pro_coord':
                    ...
                    labels, pred_pts = ConvPose(labels, ori_pts, pre, 'auto_PCA',self.device)
            ...
            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.val_fre):
                ...
                for step, (fr_val, tf_val, tf_val_inv) in enumerate(self.val_loader_reg):
                    ...
                    if self.opt.Conv_Coords == 'optimised_coord' and self.opt.img_pro_coord == 'pro_coord':
                        ...
                        ori_pts = torch.matmul(tforms_each_frame2frame0_val,torch.matmul(self.tform_calib,self.image_points)).permute(0,1,3,2)
                        pre = torch.matmul(pr_transfs_val,torch.matmul(self.tform_calib,self.image_points)).permute(0,1,3,2)

                        labels_val, pred_pts_val = ConvPose(labels_val, ori_pts, pre, 'auto_PCA',self.device)
                ...
                # add to tensorboard
                loss_reg = {'train_epoch_loss_reg_only': train_epoch_loss, 
                            
                            'epoch_loss_val_reg_only':epoch_loss_val,
                            }
    ...
    def load_best_rec_model_initial(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'best_validation_loss_model_best_221'),map_location=torch.device(self.device)))
        except:
            raise('No best model saved at the moment...')
        
    def load_rec_model00(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_epoch00000400'),map_location=torch.device(self.device)))
        except:
            raise('No best model saved at the moment...')
        
    def load_rec_model1000(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_epoch000001000_311'),map_location=torch.device(self.device)))
        except:
            raise('No best model saved at the moment...')

    def load_rec_model1600(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_epoch00001600'),map_location=torch.device(self.device)))
            self.VoxelMorph_net.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_reg_epoch00001600'),map_location=torch.device(self.device)))
    ...
    def load_best_rec_model_half_converge(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_half_converge'),map_location=torch.device(self.device)))
