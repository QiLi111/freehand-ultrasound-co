    ...
    def parse(self):
        ...
        if self.opt.CONSIATENT_LOSS == True or self.opt.ACCUMULAT_LOSS == True:
            ...
            if not self.opt.train_set_type:
                saved_results = 'seq_len' + str(self.opt.NUM_SAMPLES) + '_' + self.opt.model_name + '_' + 'lr' + str(
                    self.opt.LEARNING_RATE) + '_scan_len' + str(self.opt.MIN_SCAN_LEN)  # +'/'+'isbi'
            else:
                if self.opt.atm_pct==0:
                    saved_results = 'seq_len' + str(self.opt.NUM_SAMPLES) + '_' + self.opt.model_name + '_' + 'lr' + str(
                        self.opt.LEARNING_RATE) + '_scan_len' + str(self.opt.MIN_SCAN_LEN) + '_' + str(
                        self.opt.train_set_type)+ '_module_' + str(self.opt.branch_id)+ '_anatomy_baseline'  # +'/'+'isbi'
                elif self.opt.atm_pct==1:
                    saved_results = 'seq_len' + str(self.opt.NUM_SAMPLES) + '_' + self.opt.model_name + '_' + 'lr' + str(
                        self.opt.LEARNING_RATE) + '_scan_len' + str(self.opt.MIN_SCAN_LEN) + '_' + str(
                        self.opt.train_set_type)+ '_module_' + str(self.opt.branch_id)+ '_protocol_'+str(self.opt.class_protocol)+'_baseline'  # +'/'+'isbi'
