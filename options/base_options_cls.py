...
class BaseOptions():
    ...
    def parse(self):
        ...
        if self.opt.CONSIATENT_LOSS == True or self.opt.ACCUMULAT_LOSS == True:
            ...
            if self.opt.protocol_anatomy == 1:
                saved_results = 'seq_len' + str(self.opt.NUM_SAMPLES) + '_' + self.opt.model_name + '_' + 'lr' + str(
                    self.opt.LEARNING_RATE) + '_scan_len' + str(self.opt.MIN_SCAN_LEN)+ '_protocol_' + str(self.opt.class_protocol)  # +'/'+'isbi'
            else:
                saved_results = 'seq_len' + str(self.opt.NUM_SAMPLES) + '_' + self.opt.model_name + '_' + 'lr' + str(
                    self.opt.LEARNING_RATE) + '_scan_len' + str(self.opt.MIN_SCAN_LEN)   + '_anatomy_' + str(self.opt.class_anatomy)  # +'/'+'isbi'
