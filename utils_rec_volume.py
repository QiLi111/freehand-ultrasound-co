...
class Train_Rec_Reg_Model():
        ...
        for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.NUM_EPOCHS):
            ...
            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.val_fre):
                ...
                for step, (fr_val, tf_val, tf_val_inv) in enumerate(self.val_loader_rec):
                    ...
                    dist_val = self.metrics(pred_pts_val, labels_val).item()
                    # print(dist_val)
    ...
    def load_best_rec_model_initial(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'best_validation_loss_model_initial_best'),map_location=torch.device(self.device)))
        except:
    ...
    def load_best_rec_model_half_converge(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_half_converge_epoch1300'),map_location=torch.device(self.device)))
