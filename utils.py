...
def save_best_network(opt, model, epoch_label, running_loss_val, running_dist_val, val_loss_min, val_dist_min,
        linear_list_anatomy,linear_list_protocol,all_parameter_cls,feature_map):
    '''
    ...
    if running_loss_val < val_loss_min:
        ...
        dict_anatomy, dict_pro = [],[]
        for i_ana in range(len(linear_list_anatomy)):
            dict_anatomy.append(linear_list_anatomy[i_ana].state_dict())
            dict_pro.append(linear_list_protocol[i_ana].state_dict())

        torch.save(dict_anatomy, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_loss_anatomy'))
        torch.save(dict_pro, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_loss_protocol'))
        torch.save(all_parameter_cls, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_loss_weights'))
        torch.save(feature_map, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_loss_feature_map'))
    ...
    if running_dist_val < val_dist_min:
        ...
        torch.save(dict_anatomy, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_dist_anatomy'))
        torch.save(dict_pro, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_dist_protocol'))
        torch.save(all_parameter_cls, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_dist_weights'))
        torch.save(feature_map, os.path.join(opt.SAVE_PATH,'saved_model','best_validation_dist_feature_map'))
...
def save_best_network_cls(opt, model, epoch_label, running_loss_val, val_loss_min):
    '''
...
def add_scalars_cls_each_model(writer,epoch, loss_protocol,acc_protocol,loss_anatomy,acc_anatomy,loss_protocol_train,acc_protocol_train,loss_anatomy_train,acc_anatomy_train):
    for i in range(len(loss_protocol)):
        # classification tasks on validation set, for tarining process

        writer.add_scalars('loss_each_model', {'training_protocol_model_%s' % str(i)  : loss_protocol_train[f'module{i}']}, epoch)
        writer.add_scalars('f1_each_model', {'training_protocol_model_%s' % str(i)  : acc_protocol_train[f'module{i}']}, epoch)
        writer.add_scalars('loss_each_model', {'training_anatomy_model_%s' % str(i)  : loss_anatomy_train[f'module{i}']}, epoch)
        writer.add_scalars('f1_each_model', {'training_anatomy_model_%s' % str(i)  : acc_anatomy_train[f'module{i}']}, epoch)
        
        # classification tasks on train set, for validation process

        writer.add_scalars('loss_each_model', {'val_protocol_model_%s' % str(i)  : loss_protocol[f'module{i}']}, epoch)
        writer.add_scalars('f1_each_model', {'val_protocol_model_%s' % str(i)  : acc_protocol[f'module{i}']}, epoch)
        writer.add_scalars('loss_each_model', {'val_anatomy_model_%s' % str(i)  : loss_anatomy[f'module{i}']}, epoch)
        writer.add_scalars('f1_each_model', {'val_anatomy_model_%s' % str(i)  : acc_anatomy[f'module{i}']}, epoch)





def add_scalars_cls_weighted(writer,epoch, loss_protocol,acc_protocol,loss_anatomy,acc_anatomy,loss_protocol_train,acc_protocol_train,loss_anatomy_train,acc_anatomy_train):
    # classification tasks on validation set, for tarining process
    writer.add_scalars('loss_weighted', {'training_protocol_loss':loss_protocol_train},epoch)
    writer.add_scalars('loss_weighted', {'training_anatomy_loss':loss_anatomy_train},epoch)
    writer.add_scalars('f1_weighted', {'training_protocol_f1':acc_protocol_train}, epoch)
    writer.add_scalars('f1_weighted', {'training_anatomy_f1':acc_anatomy_train}, epoch)

    # classification tasks on train set, for validation process

    writer.add_scalars('loss_weighted', {'val_protocol_loss':loss_protocol},epoch)
    writer.add_scalars('loss_weighted', {'val_anatomy_loss':loss_anatomy},epoch)
    writer.add_scalars('f1_weighted', {'val_protocol_f1':acc_protocol}, epoch)
    writer.add_scalars('f1_weighted', {'val_anatomy_f1':acc_anatomy}, epoch)





def add_scalars_weights(writer,epoch, params_cla_protocol,params_cla_anatomy):
    for i in range(len(params_cla_protocol)):
        writer.add_scalars('weights_protocol', {'model_%s' % str(i)  : params_cla_protocol[i]}, epoch)
        writer.add_scalars('weights_anatomy', {'model_%s' % str(i)  : params_cla_anatomy[i]}, epoch)

def add_scalars_weights_acc(writer,params_cla_protocol,acc_protocol,acc_protocol_train,params_cla_anatomy,acc_anatomy,acc_anatomy_train):
    for i in range(len(params_cla_protocol)):
        # classification tasks on validation set, for tarining process
        writer.add_scalars('f1_w', {'training_protocol_model_%s' % str(i)  : acc_protocol_train[f'module{i}']}, params_cla_protocol[i])
        writer.add_scalars('f1_w', {'training_anatomy_model_%s' % str(i)  : acc_anatomy_train[f'module{i}']}, params_cla_anatomy[i])
        
        writer.add_scalars('f1_w', {'val_protocol_model_%s' % str(i)  : acc_protocol[f'module{i}']}, params_cla_protocol[i])
        writer.add_scalars('f1_w', {'val_anatomy_model_%s' % str(i)  : acc_anatomy[f'module{i}']}, params_cla_anatomy[i])


def add_scalars_weights_loss(writer,params_cla_protocol,loss_protocol,loss_protocol_train,params_cla_anatomy,loss_anatomy,loss_anatomy_train):
    for i in range(len(params_cla_protocol)):
        # classification tasks on validation set, for tarining process
        writer.add_scalars('loss_w', {'training_protocol_model_%s' % str(i)  : loss_protocol_train[f'module{i}']}, params_cla_protocol[i])
        writer.add_scalars('loss_w', {'training_anatomy_model_%s' % str(i)  : loss_anatomy_train[f'module{i}']}, params_cla_anatomy[i])
        
        writer.add_scalars('loss_w', {'val_protocol_model_%s' % str(i)  : loss_protocol[f'module{i}']}, params_cla_protocol[i])
        writer.add_scalars('loss_w', {'val_anatomy_model_%s' % str(i)  : loss_anatomy[f'module{i}']}, params_cla_anatomy[i])


def add_scalars_loss_cls(writer, epoch, loss_train,loss_val,acc_train,acc_val):
    writer.add_scalars('loss', {'train_loss': loss_train}, epoch)
    writer.add_scalars('loss', {'val_loss': loss_val}, epoch)
    writer.add_scalars('f1', {'train_f1': acc_train}, epoch)
    writer.add_scalars('f1', {'val_f1': acc_val}, epoch)
...
def sample_seperate(scan_flag):
    # seperate samples according to the anatomy and protocols, which is included in the scan_flag
    sample_subjects,sample_protocols={},{}
    subjects=list(set(list(scan_flag[0].numpy())))
    protocols = list(set(list(scan_flag[1].numpy())))
    for i in range(len(subjects)):
        temp=list(np.where(scan_flag[0].numpy() == subjects[i]))
        temp=[l.tolist() for l in temp]
        temp=np.array(temp).squeeze().tolist()
        sample_subjects[i]=temp

    for i in range(len(protocols)):
        temp=list(np.where(scan_flag[1].numpy() == protocols[i]))
        temp=[l.tolist() for l in temp]
        temp=np.array(temp).squeeze().tolist()
        sample_protocols[i]=temp



    return sample_subjects,sample_protocols

def cls_f1(branch_id,feature_map,linear_list_anatomy,cls_atm,criterion_NLL,loss_anatomy,acc_anatomy):
    # evaluate clasification performance using f1

    
    indices = [branch_id]

    feature_extracted_protocol_train, feature_flattened_protocol_train, preds_protocol_train,preds_anatomy_train = {}, {}, {}, {}
    for idx_fea_train in indices:
        if f'module{idx_fea_train}' not in feature_extracted_protocol_train.keys():
            feature_extracted_protocol_train[f'module{idx_fea_train}'] = feature_map[f'module{idx_fea_train}']
            feature_flattened_protocol_train[f'module{idx_fea_train}'] = torch.flatten(feature_map[f'module{idx_fea_train}'],start_dim=1)


            preds_anatomy_train[f'module{idx_fea_train}'] = linear_list_anatomy[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}'])
            
            if f'module{idx_fea_train}' not in loss_anatomy.keys():
                loss_anatomy[f'module{idx_fea_train}']=criterion_NLL(torch.log(preds_anatomy_train[f'module{idx_fea_train}']+ 1e-20), cls_atm).item()
            else:
                loss_anatomy[f'module{idx_fea_train}']+=criterion_NLL(torch.log(preds_anatomy_train[f'module{idx_fea_train}']+ 1e-20), cls_atm).item()

            loss_cla_atm = criterion_NLL(torch.log(preds_anatomy_train[f'module{idx_fea_train}']+ 1e-20), cls_atm)

            
            # monitor classification accuracy
            _, prediction_anatomy_train = torch.max(preds_anatomy_train[f'module{idx_fea_train}'], 1)
            # correct_predictions = torch.sum(label_protocol == prediction)
            if f'module{idx_fea_train}' not in acc_anatomy.keys():
                acc_anatomy[f'module{idx_fea_train}']=f1_score(cls_atm.cpu(),prediction_anatomy_train.cpu(), average='weighted')

            else:
                acc_anatomy[f'module{idx_fea_train}']+=f1_score(cls_atm.cpu(),prediction_anatomy_train.cpu(),average='weighted')

            
    return loss_anatomy,acc_anatomy,loss_cla_atm


def cls_f1_input_directly(branch_id,input_frame,linear_list_anatomy,cls_atm,criterion_NLL,loss_anatomy,acc_anatomy):
    # evaluate clasification performance using f1
    ...
    for idx_fea_train in indices:
        ...
        feature_flattened_protocol_train[f'module{idx_fea_train}']=torch.flatten(input_frame,start_dim=1)
        preds_anatomy_train[f'module{idx_fea_train}'] = linear_list_anatomy[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}'])
...
def cls_f1_copy(branch_id,feature_map,linear_list_protocol,acc_protocol_each_scan,cls_pro,linear_list_anatomy,acc_anatomy_each_scan,cls_atm,all_parameter_cls):
    # evaluate clasification performance using f1

    if branch_id == -1:
        indices = range(len(linear_list_protocol))
    else:
        indices = [branch_id]
    ...
    for idx_fea_train in indices:
        if f'module{idx_fea_train}' not in feature_extracted_protocol_train.keys():
            ...
            preds_protocol_train[f'module{idx_fea_train}'] = linear_list_protocol[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}'])
            if 'preds_protocol_all_train' not in globals():
                preds_protocol_all_train=torch.unsqueeze(linear_list_protocol[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}']),2)
            else:
                preds_protocol_all_train=torch.cat((preds_protocol_all_train,torch.unsqueeze(linear_list_protocol[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}']),2)),dim=2)

            
            # monitor classification accuracy
            _, prediction_train = torch.max(preds_protocol_train[f'module{idx_fea_train}'], 1)
            
            if f'module{idx_fea_train}' not in acc_protocol_each_scan.keys():
                acc_protocol_each_scan[f'module{idx_fea_train}']=[f1_score(cls_pro,prediction_train.cpu(),average='weighted')]

            else:
                acc_protocol_each_scan[f'module{idx_fea_train}'].append(f1_score(cls_pro,prediction_train.cpu(),average='weighted'))



            preds_anatomy_train[f'module{idx_fea_train}'] = linear_list_anatomy[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}'])
            if 'preds_anatomy_all_train' not in globals():
                preds_anatomy_all_train=torch.unsqueeze(linear_list_anatomy[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}']),2)
            else:
                preds_anatomy_all_train=torch.cat((preds_anatomy_all_train,torch.unsqueeze(linear_list_anatomy[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}']),2)),dim=2)
            
            
            # monitor classification accuracy
            _, prediction_anatomy_train = torch.max(preds_anatomy_train[f'module{idx_fea_train}'], 1)
            # correct_predictions = torch.sum(label_protocol == prediction)
            if f'module{idx_fea_train}' not in acc_anatomy_each_scan.keys():
                acc_anatomy_each_scan[f'module{idx_fea_train}']=[f1_score(cls_atm.cpu(),prediction_anatomy_train.cpu(), average='weighted')]

            else:
                acc_anatomy_each_scan[f'module{idx_fea_train}'].append(f1_score(cls_atm.cpu(),prediction_anatomy_train.cpu(),average='weighted'))

        if branch_id == -1:
            # compute weighted loss and accuracy 
            preds_anatomy_weighted_train = torch.matmul(preds_anatomy_all_train,F.softmax(all_parameter_cls[-2],dim=0))
            preds_protocol_weighted_train = torch.matmul(preds_protocol_all_train,F.softmax(all_parameter_cls[-1],dim=0))

            _, prediction_train = torch.max(preds_protocol_weighted_train, 1)
            
            if f'module{10}' not in acc_protocol_each_scan.keys():
                acc_protocol_each_scan[f'module{idx_fea_train}']=[f1_score( cls_pro.cpu(),prediction_train.cpu(), average='weighted')]

            else:
                acc_protocol_each_scan[f'module{idx_fea_train}'].append(f1_score(cls_pro.cpu(),prediction_train.cpu(),average='weighted'))



            _, prediction_train = torch.max(preds_anatomy_weighted_train, 1)
            if f'module{10}' not in acc_anatomy_each_scan.keys():
                acc_anatomy_each_scan[f'module{idx_fea_train}']=[f1_score(cls_atm.cpu(),prediction_train.cpu(), average='weighted')]

            else:
                acc_anatomy_each_scan[f'module{idx_fea_train}'].append(f1_score(cls_atm.cpu(),prediction_train.cpu(), average='weighted'))

                
    return acc_protocol_each_scan,acc_anatomy_each_scan


def cls_f1_acc(feature_map,linear_list_protocol,acc_protocol_each_scan,accuracy_protocol,cls_pro,weights):
    # evaluate clasification performance using f1

    indices = range(len(linear_list_protocol))
   

    feature_extracted_protocol_train, feature_flattened_protocol_train, preds_protocol_train = {}, {}, {}
    for idx_fea_train in indices:
        if f'module{idx_fea_train}' not in feature_extracted_protocol_train.keys():
            feature_extracted_protocol_train[f'module{idx_fea_train}'] = feature_map[f'module{idx_fea_train}']
            feature_flattened_protocol_train[f'module{idx_fea_train}'] = torch.flatten(feature_map[f'module{idx_fea_train}'],start_dim=1)
            ...
            preds_protocol_train[f'module{idx_fea_train}'] = linear_list_protocol[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}'])
            if 'preds_protocol_all_train' not in locals():
                preds_protocol_all_train=torch.unsqueeze(linear_list_protocol[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}']),2)
            else:
                preds_protocol_all_train=torch.cat((preds_protocol_all_train,torch.unsqueeze(linear_list_protocol[idx_fea_train](feature_flattened_protocol_train[f'module{idx_fea_train}']),2)),dim=2)
            ...
            if f'module{idx_fea_train}' not in acc_protocol_each_scan.keys():
                acc_protocol_each_scan[f'module{idx_fea_train}']=[f1_score(cls_pro.cpu(),prediction_train.cpu(),average='weighted')]
                accuracy_protocol[f'module{idx_fea_train}']=[accuracy_score(cls_pro.cpu(),prediction_train.cpu())]

            else:
                acc_protocol_each_scan[f'module{idx_fea_train}'].append(f1_score(cls_pro.cpu(),prediction_train.cpu(),average='weighted'))
                accuracy_protocol[f'module{idx_fea_train}'].append(accuracy_score(cls_pro.cpu(),prediction_train.cpu()))



            
    # compute weighted loss and accuracy 
    preds_protocol_weighted_train = torch.matmul(preds_protocol_all_train,F.softmax(weights,dim=0))

    _, prediction_train = torch.max(preds_protocol_weighted_train, 1)
    
    if f'module{-1}' not in acc_protocol_each_scan.keys():
        acc_protocol_each_scan[f'module{-1}']=[f1_score( cls_pro.cpu(),prediction_train.cpu(), average='weighted')]
        accuracy_protocol[f'module{-1}']=[accuracy_score( cls_pro.cpu(),prediction_train.cpu())]

    else:
        acc_protocol_each_scan[f'module{-1}'].append(f1_score(cls_pro.cpu(),prediction_train.cpu(),average='weighted'))
        accuracy_protocol[f'module{-1}'].append(accuracy_score(cls_pro.cpu(),prediction_train.cpu()))



                
    return acc_protocol_each_scan,accuracy_protocol
