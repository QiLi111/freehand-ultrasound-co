...
class SSFrameDataset():  # Subject-Scan frame loader
    ...
    def partition_by_ratio(self, ratios, randomise=False, subject_level=False):
        ...
        if subject_level:
            ...
            return [SSFrameDataset(min_scan_len = self.min_scan_len,data_path = self.data_path, h5_file_name=self.h5_file_name, indices_in_use=idx_list, num_samples=self.num_samples, sample_range=self.sample_range) for idx_list in indices_sets]
