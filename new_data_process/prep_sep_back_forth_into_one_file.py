...
# 3) Seperate the whole loop scan into forth and back scan, into one single file, for future training use
...
for i_sub, folder in enumerate(folders_subject):
    ...
    with open(os.path.join(os.path.join(DIR_RAW,folder),'valid_frames_sep_forth_back.csv'), 'w', encoding='UTF8') as f:
        writer = csv.writer(f)
    ...
    for i_scan, fn in enumerate(fn_mha):
        ...
        with open(os.path.join(os.path.join(DIR_RAW,folder),'valid_frames_sep_forth_back.csv'), 'a', encoding='UTF8') as f:
            writer = csv.writer(f)
        ...
        for i_frame in range(frames_forth.shape[0]):
            fh5_frames_forth.create_dataset('/sub%03d_scan%02d_frame%04d' % (i_sub,i_scan*2,i_frame), frames_forth.shape[1:3], dtype=frames_forth.dtype, data=frames_forth[i_frame,...])
            fh5_frames_forth.create_dataset('/sub%03d_scan%02d_tform%04d' % (i_sub,i_scan*2,i_frame), tforms_forth.shape[1:3], dtype=tforms_forth.dtype, data=tforms_forth[i_frame,...])
            fh5_frames_forth.create_dataset('/sub%03d_scan%02d_tform_inv%04d' % (i_sub,i_scan*2,i_frame), tforms_inv_forth.shape[1:3], dtype=tforms_inv_forth.dtype, data=tforms_inv_forth[i_frame,...])
            # if Extract_Feats:

        if FLAG_SCANFILE:
            fh5_scans_forth.create_dataset('/sub%03d_frames%02d' % (i_sub,i_scan*2), frames_forth.shape, dtype=frames_forth.dtype, data=frames_forth)
            fh5_scans_forth.create_dataset('/sub%03d_tforms%02d' % (i_sub,i_scan*2), tforms_forth.shape, dtype=tforms_forth.dtype, data=tforms_forth)
            fh5_scans_forth.create_dataset('/sub%03d_tforms_inv%02d' % (i_sub,i_scan*2), tforms_inv_forth.shape, dtype=tforms_inv_forth.dtype, data=tforms_inv_forth)


        num_frames_back[i_sub,i_scan*2+1] = frames_back.shape[0]
        name_scan_back[i_sub][i_scan*2+1] = fn+'_back'
        for i_frame in range(frames_back.shape[0]):
            fh5_frames_back.create_dataset('/sub%03d_scan%02d_frame%04d' % (i_sub,i_scan*2+1,i_frame), frames_back.shape[1:3], dtype=frames_back.dtype, data=frames_back[i_frame,...])
            fh5_frames_back.create_dataset('/sub%03d_scan%02d_tform%04d' % (i_sub,i_scan*2+1,i_frame), tforms_back.shape[1:3], dtype=tforms_back.dtype, data=tforms_back[i_frame,...])
            fh5_frames_back.create_dataset('/sub%03d_scan%02d_tform_inv%04d' % (i_sub,i_scan*2+1,i_frame), tforms_inv_back.shape[1:3], dtype=tforms_inv_back.dtype, data=tforms_inv_back[i_frame,...])
            # if Extract_Feats:

        if FLAG_SCANFILE:
            fh5_scans_back.create_dataset('/sub%03d_frames%02d' % (i_sub,i_scan*2+1), frames_back.shape, dtype=frames_back.dtype, data=frames_back)
            fh5_scans_back.create_dataset('/sub%03d_tforms%02d' % (i_sub,i_scan*2+1), tforms_back.shape, dtype=tforms_back.dtype, data=tforms_back)
            fh5_scans_back.create_dataset('/sub%03d_tforms_inv%02d' % (i_sub,i_scan*2+1), tforms_inv_back.shape, dtype=tforms_inv_back.dtype, data=tforms_inv_back)
