...
fh5_frames_forth = h5py.File(os.path.join(PATH_SAVE,'frames_res{}_cls_forth'.format(RESAMPLE_FACTOR)+".h5"),'a')
if FLAG_SCANFILE:
    fh5_scans_forth = h5py.File(os.path.join(PATH_SAVE,'scans_res{}_cls_forth'.format(RESAMPLE_FACTOR)+".h5"),'a')

fh5_frames_back = h5py.File(os.path.join(PATH_SAVE,'frames_res{}_cls_back'.format(RESAMPLE_FACTOR)+".h5"),'a')
if FLAG_SCANFILE:
    fh5_scans_back = h5py.File(os.path.join(PATH_SAVE,'scans_res{}_cls_back'.format(RESAMPLE_FACTOR)+".h5"),'a')
...
cls_protocol_forth = np.array([0,1,2])
...
cls_protocol_back = np.array([0,1,2])
...
for i_sub, folder in enumerate(folders_subject):
    ...
    for i_scan, fn in enumerate(fn_mha):
        ...
        if 'L' in name_scan_forth[i_sub][i_scan]:
            protocol_cls_forth = cls_protocol_forth[0]

        elif 'C' in name_scan_forth[i_sub][i_scan]:
            protocol_cls_forth = cls_protocol_forth[1]
        elif 'S' in name_scan_forth[i_sub][i_scan]:
        ...
        # generate classification labels
        if 'L' in name_scan_back[i_sub][i_scan]:
            protocol_cls_back = cls_protocol_back[0]

        elif 'C' in name_scan_back[i_sub][i_scan]:
            protocol_cls_back = cls_protocol_back[1]
        elif 'S' in name_scan_back[i_sub][i_scan]:
