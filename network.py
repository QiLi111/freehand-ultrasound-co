...
def build_model(opt,in_frames, pred_dim, label_dim, image_points, tform_calib, tform_calib_R_T,input_size=(120,160)):
    """
    ...
    if opt.model_name == "efficientnet_b1":
        ...
        # need to put input to model, then the feature_map can have values
        test_tensor=torch.ones(1,in_frames,*input_size)
        model(test_tensor)

        feature_extracted_protocol,feature_flattened_protocol={},{}
        linear_list_anatomy, linear_list_protocol = [],[]
        for idx_fea in range(len(feature_map)):
            # temp = feature_map[f'module{idx_fea}']
            temp = torch.flatten(feature_map[f'module{idx_fea}'], start_dim=1)
            # size2 = int(temp.size()[1]/(math.sqrt(temp.size()[1]/2**6)*1.5))
            size1 = 2**7
            # size3 = int(temp.size()[1]/2)

            # linear1 = nn.Linear(temp.size()[1], size1)
            # RL1 = nn.ReLU()
            # linear2 = nn.Linear(size1, size2)
            # RL2 = nn.ReLU()
            # linear3 = nn.Linear(size2, opt.class_anatomy)
            SM = nn.Softmax(dim=1)
            # linear_layers = nn.Sequential(linear1,RL1,linear2, RL2,linear3,SM)
            
            # linear1 = nn.Linear(temp.size()[1], size1)
            # RL = nn.ReLU()
            # linear2 = nn.Linear(size1, opt.class_anatomy)

            # linear_layers = nn.Sequential(linear1,RL,linear2,SM)

            linear = nn.Linear(temp.size()[1], opt.class_anatomy)
            linear_layers = nn.Sequential(linear,SM)

            linear_list_anatomy.append(linear_layers)
        ...
        for idx_fea in range(len(feature_map)):
            ...
            # linear2 = nn.Linear(size1, size2)
            # RL2 = nn.ReLU()
            # linear3 = nn.Linear(size2, opt.class_protocol)
            SM = nn.Softmax(dim=1)
            # linear_layers = nn.Sequential(linear1,RL1, linear2,RL2, linear3,SM)
            
            # linear1 = nn.Linear(temp.size()[1], size1)
            # RL = nn.ReLU()
            # linear2 = nn.Linear(size1, opt.class_protocol)

            # linear_layers = nn.Sequential(linear1,RL,linear2,SM)

            linear = nn.Linear(temp.size()[1], opt.class_protocol)
            linear_layers = nn.Sequential(linear,SM)


            linear_list_protocol.append(linear_layers)


        return model, linear_list_anatomy, linear_list_protocol,feature_map
        #     get features maps of each  model.feature
        # architecture = [model.features[i] for i in range(len(model.features))]

    # initilize linear layers
        # feature_extracted_protocol,feature_flattened_protocol={},{}
        # # models output of each modual in efficient net b1
        # # 32,32,60,80//32,16,60,80//32,24,30,40//32,40,15,20//
        # # 32,80,8,10//32,112,8,10//32,192,4,5//32,320,4,5//32,1280,4,5
        # linear_list_protocol, linear_list_anatomy = {}, {}
        # for idx_fea in range(len(model.features)):
        
        #     linear1 = nn.Linear(temp.size()[1], int(temp.size()[1] / 100))
        #     linear2 = nn.Linear(int(temp.size()[1] / 100), int(temp.size()[1] / 10000))
        #     linear3 = nn.Linear(int(temp.size()[1] / 10000), opt.class_protocol)
        
        #     linear_layers = [linear1, linear2, linear3]
        #     if f'module{idx_fea}' not in linear_list_protocol.keys():
        #         linear_list_protocol[f'module{idx_fea}'] = linear_layers
        ...
        #     linear_layers = [linear1, linear2, linear3]
        #     if f'module{idx_fea}' not in linear_list_anatomy.keys():
        #         linear_list_anatomy[f'module{idx_fea}'] = linear_layers

    if opt.model_name == "efficientnet_b6":
