...
from utils_isbi import add_scalars_rec_volume,add_scalars_reg,save_best_network,save_best_network_reg,add_scalars_wrap_dist,add_scalars_reg_T
...
class Train_Rec_Reg_Model():
    ...
    def train_rec_model(self):
        ...
        # for epoch in range(int(self.opt.retain_epoch)+self.current_epoch, int(self.opt.retain_epoch)+self.current_epoch+self.opt.max_rec_epoch_each_interation):
        for epoch in range(int(self.opt.retain_epoch), int(self.opt.retain_epoch)+self.opt.NUM_EPOCHS):
            ...
            for step, (frames, tforms, tforms_inv) in enumerate(self.train_loader_rec):
                ...
                # self.metrics(pred_pts, labels).item()
                # print(loss2)
                # print(dist)
                # for i in range(4):
                #     save2mha(gt_volume[i,...].cpu().numpy(),sx = 1,sy=1,sz=1,
                #         save_folder='gt_ori_%d.mha'%i
                #         )
                #     save2mha(pred_volume[i,...].detach().cpu().numpy(),sx = 1,sy=1,sz=1,
                #         save_folder='pred_ori_%d.mha'%i
                #         )
            ...
            if epoch in range(0, self.opt.NUM_EPOCHS, self.opt.val_fre):
                ...
                # when should save the best model - either from train loss, val loss, T model or R model,
                # the model should be saved as long as the loss decrease
                # save models based on the wrapped prediction from R
                # self.save_best_models_train_T(epoch,train_epoch_wrap_dist)
                # self.save_best_models_train_R(epoch,train_epoch_wrap_dist_reg)
                # save model based on validation process
                ...
                # self.val_loss_min_reg,count_non_improved_loss_reg = save_best_network_reg(self.opt, self.VoxelMorph_net, epoch, epoch_loss_val_reg_only, self.val_loss_min_reg,count_non_improved_loss_reg)
    ...
    def load_best_rec_model_half_converge(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_half_converge_epoch400_221'),map_location=torch.device(self.device)))
            

        except:
            raise('No best model saved at the moment...')
        
    def load_rec_model8300(self):
        # load the best registation model registartion
        try:
            self.model.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_epoch00008300'),map_location=torch.device(self.device)))
            self.VoxelMorph_net.load_state_dict(torch.load(os.path.join(self.opt.SAVE_PATH,'saved_model', 'model_reg_epoch00008300'),map_location=torch.device(self.device)))
